...
def hamiltonian(p, q, log_probability, inverse_metric=None):
    ...
    return kinetic_energy(p, inverse_metric) - log_probability(q)  #+ 0.5*logdet 
...
def run_rmhmc(q0, n_steps, n_leaps, step_size, log_probability, bounds, boundary_conditions, *args, **kwargs):
    ...
    while i < n_steps:
        ...
        p0 = random.multivariate_normal(key, mean=jnp.zeros(q0.shape[0]), cov=inverse_mass_matrix_0)
        # print("p0",p0)
        # p0 = jnp.where(random.uniform(key, shape=q0.shape[0]) >0.4,  p0*1000, p0)
        # print("p0dopo",p0)
...
#            print("pre - leap ",k,"p:",p_,"q:",q_,"invM:",compute_mass_matrix(jax.hessian(log_probability),q_)[1])
            ...
            p_values.append(p_)
            q_values.append(q_)
            p, q, g, = leap_frog(step_size, p_, q_, log_probability, bounds, boundary_conditions)
...
#            print("post - leap ",k,"p:",p_,"q:",q_,"invM:",g_)
        ...
        if (alpha) > jnp.log(random.uniform(key, shape=(1,), minval=0, maxval=1)):
            ...
            # inverse_mass_matrix_0 = jax.random.choice(key, jnp.array([inverse_mass_matrix_0, inverse_mass_matrix]), p=jnp.array([0.5, 0.5]))
            print("accepted", q0)
            print('step size',np.linalg.det(inverse_mass_matrix_0), )
            # step_size = np.linalg.det(inverse_mass_matrix_0)
...
def leap_frog(dt, p0, q0, logp, bounds,boundary_conditions ):
    ...
    # q += dt * jnp.dot(jnp.linalg.inv(metric), p)  # Full-step position update
...
#    rng = [np.random.default_rng(seed = 11+j) for j in range(n_processes)]
    ...
    q0  = rng.uniform(-5,5,size=dim)#
    
    from scipy.stats import random_correlation
    
    eigs        = rng.uniform(1,100,len(q0))
    eigs        = np.array(len(q0)*eigs/np.sum(eigs))
    cov         = random_correlation.rvs(eigs, random_state=rng)
    inv_cov     = np.linalg.inv(cov)
    data        = rng.uniform(-5,5,size=dim)
    
    def log_posterior(q, data, inv_cov):
        r = (data - q)
        return -0.5*jnp.dot(r.T,jnp.dot(inv_cov,r))
    
    eigs1        = rng.uniform(1,100,len(q0))
    eigs1        = np.array(len(q0)*eigs1/np.sum(eigs1))
    cov1         = random_correlation.rvs(eigs1, random_state=rng)
    inv_cov1     = np.linalg.inv(cov)
    data1        = jnp.array([3.0]*dim)
    
    eigs2        = rng.uniform(1,10,len(q0))
    eigs2        = np.array(len(q0)*eigs/np.sum(eigs))
    cov2         = random_correlation.rvs(eigs, random_state=rng)
    inv_cov2     = np.linalg.inv(cov2)
    data2        = jnp.array([-3.0]*dim)
...
#    exit()
    ...
    chains = [run_rmhmc(q0,  n_steps, n_leaps,step_size, logp, bounds) for j in range(n_processes)]
    ...
    corner(qs, labels=None, quantiles=[0.05, 0.5, 0.95], truths = data,
